...
class YOLOXHeadSingleSplit(AnchorHeadTemplate):
    def __init__(self,
                 model_cfg,
                 input_channels,
                 num_class,
    ...
    def _init_layers(self):
        ...
        self.stacked_convs_z = self._build_stacked_convs()
    ...
    def _build_predictor(self):
        ...
        self.predictor_box = nn.Conv2d(self.feat_channels,
                                       self.num_anchors_per_location *
                                       self.box_coder.code_size-1,
                                       kernel_size=1)
        self.predictor_z = nn.Conv2d(self.feat_channels,
                                       self.num_anchors_per_location *
                                       1,
                                       kernel_size=1)
    ...
    def build_losses(self, losses_cfg):
        ...
        self.add_module(
            'z_loss_func',
            loss_utils.WeightedSmoothL1Loss(code_weights=losses_cfg.LOSS_WEIGHTS['code_weights_z'])
        )
        self.add_module(
            'obj_loss_func',
            loss_utils.WeightedCrossEntropyLoss(use_sigmoid=True)
    ...
    def get_z_reg_layer_loss(self):
        z_preds = self.forward_ret_dict['z_preds']
        z_reg_targets = self.forward_ret_dict['box_reg_targets'][:,:,2:3]
        box_cls_labels = self.forward_ret_dict['box_cls_labels']
        batch_size = int(z_preds.shape[0])
        ...
        z_preds = z_preds.view(batch_size, -1,
                                   z_preds.shape[-1] // self.num_anchors_per_location if not self.use_multihead else
                                   z_preds.shape[-1])

        z_loss_src = self.z_loss_func(z_preds, z_reg_targets, weights=reg_weights)  # [N, M]
        z_loss = z_loss_src.sum() / batch_size

        z_loss = z_loss * self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['z_weight']
        tb_dict = {
            'rpn_loss_z': z_loss.item()
        }

        return z_loss, tb_dict
    ...
    def get_box_reg_layer_loss(self):
        ...
        box_reg_targets = self.forward_ret_dict['box_reg_targets'][:,:,[0,1,3,4,5,6]]
        box_cls_labels = self.forward_ret_dict['box_cls_labels']
        ...
        anchors=anchors[:,:,[0,1,3,4,5,6]]
        box_preds = box_preds.view(batch_size, -1,
                                   box_preds.shape[-1] // self.num_anchors_per_location if not self.use_multihead else
                                   box_preds.shape[-1])
        # sin(a - b) = sinacosb-cosasinb
        box_preds_sin, reg_targets_sin = self.add_sin_difference(box_preds, box_reg_targets,dim=5)
        ...
        if box_dir_cls_preds is not None:
            dir_targets = self.get_direction_targetV2(
    ...
    def get_loss(self):
        ...
        z_loss,tb_dict_z=self.get_z_reg_layer_loss()
        tb_dict.update(tb_dict_z)

        obj_loss, tb_dict_obj = self.get_obj_layer_loss()
        tb_dict.update(tb_dict_obj)

        rpn_loss = cls_loss + box_loss + obj_loss+z_loss

        tb_dict['rpn_loss'] = rpn_loss.item()
        return rpn_loss, tb_dict

    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, obj_preds, dir_cls_preds=None,z_preds=None):
        """
        Args:
            batch_size:
            cls_preds: (N, H, W, C1)
        ...
        batch_z_preds=z_preds.view(batch_size,num_anchors,-1) if not isinstance(z_preds,list)\
            else torch.cat(z_preds,dim=1).view(batch_size,num_anchors,-1)
        batch_box_preds=torch.cat([batch_box_preds[:,:,:2],batch_z_preds,batch_box_preds[:,:,2:]],dim=-1)
    ...
    def forward(self, data_dict):
        ...
        if self.stacked_convs > 0:
            ...
            z_feat = self.stacked_convs_z(x)
        else:
            cls_feat, reg_feat,z_feat = x, x, x
        ...
        z_preds=self.predictor_z(z_feat)
