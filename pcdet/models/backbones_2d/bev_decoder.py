...
class CrossViewMaskFuseBEVDecoder(BaseBEVDecoder):
    def __init__(self, model_cfg, input_channels):
        super().__init__(model_cfg=model_cfg, input_channels=input_channels)
        self.model_cfg = model_cfg
        self.feature_names = model_cfg.get('FEATURE_NAMES', [])
        ...
        self.deblocks_bev = nn.ModuleList()
        self.deblocks_cyv = nn.ModuleList()
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.num_levels):
            ...
            if len(self.deblocks_cyv) > 0:
                ups_cyv.append(self.deblocks_cyv[i](x1))
            else:
                ups_cyv.append(x1)
            
            if len(self.deblocks_bev) > 0:
                ups_bev.append(self.deblocks_bev[i](x2))
            else:
                ups_cyv.append(x2)

        if len(ups_cyv) > 1:
            x1 = torch.cat(ups_cyv, dim=1)
        elif len(ups_cyv) == 1:
            x1 = ups_cyv[0]

        if len(ups_bev) > 1:
            x2 = torch.cat(ups_bev, dim=1)
        elif len(ups_bev) == 1:
            x2 = ups_bev[0]

        if len(self.deblocks_cyv) > self.num_levels:
            x1 = self.deblocks_cyv[-1](x1)
        if len(self.deblocks_bev) > self.num_levels:
            x2 = self.deblocks_bev[-1](x2)

        mask_cyv = self.binary_cls_cyv(x1)
        mask_bev = self.binary_cls_bev(x2)

        self.forward_ret_dict["mask_cyv"] = mask_cyv
        self.forward_ret_dict["mask_bev"] = mask_bev

        if self.training:
            gt_mask = self.mask_generate.generate(mask_cyv, data_dict["points"], data_dict['voxel_coords'], 
                                        self.voxel_size, self.point_clout_range, data_dict["gt_boxes"])
            self.forward_ret_dict["gt_mask"] = gt_mask.to(mask_cyv.device).unsqueeze(1)
        
        mask = torch.cat([mask_cyv, mask_bev], dim=1)
        mask = nn.Softmax(dim=1)(mask)

        x1 = torch.mul(x1, mask[:, :1]) + x1 * 0.5
        x2 = torch.mul(x2, mask[:, 1:]) + x2 * 0.5

        data_dict['spatial_features_2d'] = x1 + x2
        return data_dict

    def get_loss(self):
        prediction_cyv = self.forward_ret_dict["mask_cyv"]
        prediction_bev = self.forward_ret_dict["mask_bev"]

        target = self.forward_ret_dict["gt_mask"]
        tb_dict = {"loss_heatmap":0}

        for prediction in [prediction_cyv, prediction_bev]:
            self.alpha = 2
            self.beta = 4
            positive_index = target.eq(1).float()
            negative_index = target.lt(1).float()
