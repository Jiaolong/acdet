...
class RangeToBEV(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.fuse_complete_points = self.model_cfg.get(
            'FUSE_COMPLETE_POINTS', False)
    ...
    def forward(self, data_dict):
        ...
        if self.fuse_complete_points:
            batch_complete_features = self.complete_net(
                batch_complete_points[:, 1:5])
        ...
        points_batch = points_img[:, :4].permute(
            0, 2, 3, 1).contiguous()  # B, H, W, 4
        ...
        for k in range(batch_size):
            ...
            if self.fuse_complete_points:
                ...
                complete_bev_features = self.bev_projector.get_projected_features(
                    complete_points, complete_features)
                bev_features = torch.cat(
                    [bev_features, complete_bev_features], dim=0)

            bev_features_batch.append(bev_features)

        bev_features_batch = torch.stack(
            bev_features_batch)  # B, C + 4, H2, W2
